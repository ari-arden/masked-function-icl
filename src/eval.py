...
from masked_objectives import build_query_only_mask
...
def _is_masked_encoder(model):
    return getattr(model, "supports_masked_y", False) or getattr(
        model, "name", ""
    ).startswith(("masked_encoder", "masked_pair_encoder"))
    ...
    if _is_masked_encoder(model):
        ...
        for i in range(n_points):
            if xs_p is None:
                ...
                xs_cur = torch.cat((xs[:, :i, :], xs_p[:, i : i + 1, :]), dim=1)
            ys_cur = task.evaluate(xs_cur)
            query_positions = torch.full((b_size,), xs_cur.shape[1] - 1, dtype=torch.long)
            observed_y_mask, target_y_mask = build_query_only_mask(
                ys_cur, query_positions
            )
            pred = model(
                xs_cur.to(device),
                ys_cur.to(device),
                observed_y_mask=observed_y_mask.to(device),
                target_y_mask=target_y_mask.to(device),
            ).detach()
            metrics[:, i] = task.get_metric()(pred.cpu(), ys_cur)[:, -1]
        return metrics
