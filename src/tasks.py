...
class NoisyLinearRegression(LinearRegression):
    def __init__(
        ...
        """Linear regression with additive Gaussian label noise.
        ...
        renormalize_ys:
            Backward-compatible switch for the original batch-level label
            renormalization. New scripts should prefer the explicit
            ``renormalization`` argument.
        renormalization:
            ``"none"`` leaves noisy labels unscaled. ``"batch"`` matches the
            original implementation and rescales by the empirical standard
            deviation of the full generated label tensor. ``"population"``
            rescales by the analytic unconditional label standard deviation,
            avoiding dependence on evaluation-batch labels.
        """
        ...
        if renormalization is None:
            renormalization = "batch" if renormalize_ys else "none"
        if renormalization not in {"none", "batch", "population"}:
            raise ValueError(
                "renormalization must be one of 'none', 'batch', or 'population'"
            )
    ...
    def evaluate(self, xs_b):
        ...
        if self.renormalization == "batch":
            ys_b_noisy = ys_b_noisy * math.sqrt(self.n_dims) / ys_b_noisy.std()
        elif self.renormalization == "population":
            population_std = math.sqrt((self.scale**2) * self.n_dims + self.noise_std**2)
            ys_b_noisy = ys_b_noisy * math.sqrt(self.n_dims) / population_std
